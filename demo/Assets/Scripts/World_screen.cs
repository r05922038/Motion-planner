...
using System.Linq;
public class World_screen : MonoBehaviour {
    /* data_structure */
...
//			GUI.Label (new Rect (Screen.width-350, 85, 200, 20), "Frequency of lower obstacles：1/" + (int)frequenceLowerO, myGUI);
			...
			if(scoreNPC>scoreUser)
				GUI.Label (new Rect (Screen.width/4, Screen.height/2-30, Screen.width/2, 60), "You lost",myGUI);
			if(scoreNPC<scoreUser)
...
//                Futile.stage.AddChild(sGRPolygons [a][b]);
        ...
        for (int a=0; a<numObstacle; a++){
            for (int b=0; b<Obstacles[a].numPolygon; b++){
				...
				sOPolygons[a][b].color = (a==0)?Color.yellow:Color.yellow;//black,white
				...
				if(!testBFS){
					...
					if(threeTimes<2){
						...
						if(rNumPolygon==Robots[0].numPolygon)
							sGoal[threeTimes][Robots[0].numPolygon-1].color=Color.white;//yellow
					}
					else{
						for (int b=0; b<Robots[0].numPolygon; b++)
