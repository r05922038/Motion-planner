...
using System.Linq;
public class World_demo : MonoBehaviour {
    /* data_structure */
	public int numRobot,numObstacle,numAddO,totalNumAddo,frequenceLowerO,threeTimes,maxAddO,scoreNPC,scoreUser;
    public Robot[] Robots;
    public Obstacle[] Obstacles;
    public Wall Walls;
	public bool inMouse,delayByO,fromBFS,testBFS,lockMoveO;
...
//				Futile.stage.RemoveChild(sGRPolygons [a][b]);
	...
	void Start () {
		...
		fp.AddResolutionLevel(480f, 1.0f, 1.0f, "-res1");//-res1
...
//        ifShowPath = false;
	...
	void OnGUI() {		
		//		GUI.DrawTexture (new Rect (0, Screen.height - 600, 770, 196), background);
		/*		if (GUI.Button (new Rect (580, Screen.height - 540, 60, 30), "Start")) {
			getBitmap ();
			if (BFS ())
				StartCoroutine (animation (1f / 60f));
		}*/

		if (!ifAniPath) {

			if (GUI.Button (new Rect (0, Screen.height - 60, 60, 60), "GO")) {
				getBitmap ();
				if ((testBFS = BFS ())) {
					InvokeRepeating ("timer15s", 8f, 8f);
					StartCoroutine (animation (1f / aniSpeed));
				}
			}
			if (GUI.Button (new Rect (490, Screen.height - 30, 50, 30), "Reset")) {
				destroySprite ();
				reset ();
			}
			
			aniSpeed = GUI.HorizontalSlider (new Rect (460, Screen.height - 530, 70, 15), aniSpeed, 30.0F, 120.0F);
			maxAddO = (int)GUI.HorizontalSlider (new Rect (460, Screen.height - 515, 70, 15), maxAddO, 1.0F, 20.0F);
			sizeOfObstacle = GUI.HorizontalSlider (new Rect (460, Screen.height - 500, 70, 15), sizeOfObstacle, 1.5F, 6.0F);
			maxFootHeight = GUI.HorizontalSlider (new Rect (460, Screen.height - 485, 70, 15), maxFootHeight, 10.0F, 50.0F);
			frequenceLowerO = (int)GUI.HorizontalSlider (new Rect (460, Screen.height - 470, 70, 15), frequenceLowerO, 1.0F, 10.0F);
			
			myGUI.normal.textColor=Color.black;
			myGUI.fontStyle=FontStyle.Normal;
			myGUI.fontSize = 12;			
			GUI.Label (new Rect (250, Screen.height - 533, 200, 15), "NPC speed (sec/step)：1/" + (int)aniSpeed, myGUI);  
			GUI.Label (new Rect (250, Screen.height - 518, 200, 15), "Maximum # of obstacle：" + (int)maxAddO, myGUI); 
			GUI.Label (new Rect (250, Screen.height - 503, 200, 15), "Size of obstacle：" + sizeOfObstacle.ToString ("0.0") + " X " + sizeOfObstacle.ToString ("0.0"), myGUI); 
			GUI.Label (new Rect (250, Screen.height - 488, 200, 15), "Step height of NPC (cm)：" + (int)maxFootHeight, myGUI);
			GUI.Label (new Rect (250, Screen.height - 473, 200, 15), "Frequency of lower obstacles：1/" + (int)frequenceLowerO, myGUI);

			myGUI.fontSize = 100;
			myGUI.normal.textColor=Color.yellow;
			myGUI.fontStyle=FontStyle.BoldAndItalic;
			if(scoreNPC>scoreUser)
				GUI.Label (new Rect (60, Screen.height - 300, 300, 60), "You lost",myGUI);
//			myGUI.normal.textColor=Color.yellow;
			if(scoreNPC<scoreUser)
				GUI.Label (new Rect (70, Screen.height - 300, 300, 60), "You win",myGUI);



		} else {
			myGUI.fontSize = 20;
			myGUI.normal.textColor=Color.blue;
			myGUI.fontStyle=FontStyle.Bold;
			GUI.Label (new Rect (380, Screen.height - 530, 100, 25), "NPC score：" + scoreNPC,myGUI);
			GUI.Label (new Rect (380, Screen.height - 500, 100, 25), "your score：" +scoreUser,myGUI);
		}
...
//                    toGPrint [c].Set(Robots [a].Polygons [b].plannerGVertices [c].x * x_PlannerToCanvas,Robots [a].Polygons [b].plannerGVertices [c].y * y_PlannerToCanvas);
				...
				sIRPolygons [a][b].color = (b==0)?Color.blue:Color.cyan;
...
//                Futile.stage.AddChild(sGRPolygons [a][b]);
		...
		for (int a=0; a<3; a++) {
			...
			for (int b=0; b<Robots[0].numPolygon; b++) {
				...
				sGoal [a] [b].color = (a==0)?Color.gray:Color.white;
        ...
        for (int a=0; a<numObstacle; a++){
            for (int b=0; b<Obstacles[a].numPolygon; b++){
				...
				sOPolygons[a][b].color = (a==0)?Color.yellow:Color.black;
			...
			if (detection){
				for (i1=0; i1<Me_O.Length; i1++){
					for (i2=0; i2<Me_O[i1].Length; i2++){
						...
						for (i_polygon=0; i_polygon<Others_O[i_others].Length; i_polygon++){
...
//		for(i_others=0, detection=false ; i_others < OthersBounder.Length; i_others++, detection=false){
		for(i_others=OthersBounder.Length-1; i_others >=0; i_others--){
			detection=false;
...
/*    public void clearPATH(){       
    ...
    public bool BFS(){
        ...
        if ((testAngle = equalsGoal(cur.pos,cur.ang)) < 360){
            if (Mathf.Abs(testAngle) <= 6 || Mathf.Abs(testAngle) >= 354){
                Success = true;
            ...
            for (i_child=1; i_child<=6; i_child++,i_valid=false){
                switch (i_child){
                    ...
                    case 4:/* y-1 */
                        ...
                        break;
                    case 5:/* Angle+4 */                        
                        childAngle = Normalize(cur.ang + 6);
...
//            print("the number of nodes in pathT : "+pathT.Count());
    ...
    IEnumerator animation(float timeinseconds){
		...
		while(threeTimes<3){
			...
			//Me,MyBounder,Others
			Me = new Vector2[Robots [0].numPolygon][];
			for (int j=0; j<Robots[0].numPolygon; j++)
				Me [j] = new Vector2[Robots [thRobot].Polygons [j].numVertice];
			MyBounder = new Vector2[2];				  
			robotBuildOthers ();
			int i;
			if(testBFS){
...
//							print ("ok");
					...
					yield return new WaitForSeconds (delayByO?timeinseconds*timerPenalty*3:timeinseconds*timerPenalty);
				}
			}
			if(ifAniCo||(!testBFS)){
				while(testBFS&&(vertify>0)){
					getBitmap ();
					if((testBFS=BFS()))
						break;
					vertify--;
				}
				if((vertify==0)||!testBFS){
					scoreUser++;
					if(threeTimes<2){
						threeTimes++;
						...
						for (int b=0; b<Robots[0].numPolygon; b++) {
							sGoal[threeTimes-1][b].color=Color.white;
